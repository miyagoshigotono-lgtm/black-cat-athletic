 ...
 *   B. 倒木（右手）：倒木を登り、④の木の幹で止まる所(1.45) → ④の枝(1.95〜2.3) → 横へ跳んで⑤の枝(2.5) → 枝先から塀を跳び越える
 *   C. 岩（左手）：岩A(0.56) → 岩B(1.1) → 岩C(1.7) → 岩D(2.0) → 塀ぎわの岩(2.1) → 塀(2.4)を跳び越える
...
solids.push({
  kind: 'beam', name: '①の枝（西）', p1: [0.2, 2.1, 2.6], p2: [-1.0, 2.42, 1.2], width: 0.22, thickness: 0.16, color: 'branch',
  attachedTo: ['①の木・幹'],
});
...
solids.push({
  kind: 'beam', name: '②の枝（北）', p1: [-2.0, 2.6, 0.4], p2: [-2.55, 2.95, -1.75], width: 0.22, thickness: 0.16, color: 'branch',
  attachedTo: ['②の木・幹'],
});
...
solids.push({
  // ④の木の幹に立てかけた倒木（傾き約 19°）。幹に当たって止まる所（④の枝と同じ側）から枝へ跳ぶ
  kind: 'beam', name: '倒木', p1: [4.4, 0.35, 2.4], p2: [3.0, 1.5, -0.55], width: 0.38, thickness: 0.35, color: 'bark',
  attachedTo: ['④の木・幹'],
});
solids.push({
  kind: 'beam', name: '④の枝', p1: [2.9, 1.95, -0.5], p2: [3.35, 2.3, -3.3], width: 0.25, thickness: 0.16, color: 'branch',
  attachedTo: ['④の木・幹'],
});
solids.push({
...
rock('岩B', -4.8, -1.75, 0.65, 0.55); // 1.10
rock('岩C', -4.5, -2.9, 0.65, 0.85); // 1.70
rock('岩D', -5.05, -4.0, 0.6, 1.0); // 2.00
rock('塀ぎわの岩', -4.7, -5.1, 0.6, 1.05); // 2.10 → 塀の上（2.4）へ
